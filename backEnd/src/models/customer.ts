...
class Customer extends Model<CustomerAttributes, CustomerCreationAttributes> implements CustomerAttributes {
    ...
    static async updateCustomer(updatedData: Partial<CustomerAttributes>): Promise<[number, Customer[]]> {
        try {
            const [updatedRowsCount, updatedCustomer] = await Customer.update(updatedData, {
                where: { customer_id: updatedData.customer_id },
                returning: true,
            });

            if (updatedRowsCount === 0 || !updatedCustomer || updatedCustomer.length === 0) {
